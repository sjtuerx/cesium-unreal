 ...
 *
 * Constants that can be passed to a {@link ThirdPartyMapsRasterOverlay} to
 * indicate the overlays that should be painted.
 */
struct ThirdPartyMapsStyle final {
  /**
   ...
   * Use `ThirdPartyMapsStyle.AERIAL_WITH_LABELS_ON_DEMAND` instead
   */
   ...
   * @deprecated See https://github.com/CesiumGS/cesium/issues/7128.
   * Use `ThirdPartyMapsStyle.ROAD_ON_DEMAND` instead
   */
 ...
 * imagery data.
 */
class CESIUMRUNTIME_API ThirdPartyMapsRasterOverlay
...
public:
   ...
   * {@link ThirdPartyMapsStyle}, with {@link ThirdPartyMapsStyle::AERIAL} being the
   * default.
  ...
  virtual ~ThirdPartyMapsRasterOverlay() override;
 ...
 * imagery data.
 */
class CESIUMRUNTIME_API FBingMapsRasterOverlay
    : public ThirdPartyMapsRasterOverlay {
public:
  /**
   * @brief Creates a new instance.
   *
   ...
   * https://www.bingmapsportal.com/.
   * @param mapStyle The type of Bing Maps imagery to load. A value from
   * {@link ThirdPartyMapsStyle}, with {@link ThirdPartyMapsStyle::AERIAL} being
   * the default.
   * @param culture The culture to use when requesting Bing Maps imagery. Not
   * all cultures are supported. See
   * http://msdn.microsoft.com/en-us/library/hh441729.aspx for information on
  ...
  FBingMapsRasterOverlay(
      ...
      const CesiumGeospatial::Ellipsoid& ellipsoid =
          CesiumGeospatial::Ellipsoid::WGS84,
      const RasterOverlayOptions& overlayOptions = {});
  virtual ~FBingMapsRasterOverlay() override {}
