//#include "Cesium3DTilesSelection/ThirdPartyMapsRasterOverlay.h"
...
#pragma region ThirdPartyMaps

const std::string ThirdPartyMapsStyle::AERIAL = "Aerial";
const std::string ThirdPartyMapsStyle::AERIAL_WITH_LABELS = "AerialWithLabels";
const std::string ThirdPartyMapsStyle::AERIAL_WITH_LABELS_ON_DEMAND =
    "AerialWithLabelsOnDemand";
const std::string ThirdPartyMapsStyle::ROAD = "Road";
const std::string ThirdPartyMapsStyle::ROAD_ON_DEMAND = "RoadOnDemand";
const std::string ThirdPartyMapsStyle::CANVAS_DARK = "CanvasDark";
const std::string ThirdPartyMapsStyle::CANVAS_LIGHT = "CanvasLight";
const std::string ThirdPartyMapsStyle::CANVAS_GRAY = "CanvasGray";
const std::string ThirdPartyMapsStyle::ORDNANCE_SURVEY = "OrdnanceSurvey";
const std::string ThirdPartyMapsStyle::COLLINS_BART = "CollinsBart";

const std::string ThirdPartyMapsRasterOverlay::BING_LOGO_HTML =
...
class ThirdPartyMapsTileProvider final : public QuadtreeRasterOverlayTileProvider {
public:
  ThirdPartyMapsTileProvider(
      RasterOverlay& owner,
  ...
  virtual ~ThirdPartyMapsTileProvider() {}
  ...
  virtual CesiumAsync::Future<LoadedRasterOverlayImage> loadQuadtreeTileImage(
    ...
    std::string url = CesiumUtility::Uri::substituteTemplateParameters(
        ...
        [this, &tileID](const std::string& key) {
          if (key == "quadkey") {
            return ThirdPartyMapsTileProvider::tileXYToQuadKey(
                tileID.level,
                tileID.x,
...
ThirdPartyMapsRasterOverlay::ThirdPartyMapsRasterOverlay(
...
ThirdPartyMapsRasterOverlay::createTileProvider(
...
/*              "{}",
    ...
    return std::make_unique<ThirdPartyMapsTileProvider>(
        *pOwner,
        asyncSystem,
        pAssetAccessor,
        bingCredit,
...
#pragma endregion ThirdPartyMaps


#pragma region BingMaps

 FBingMapsRasterOverlay::FBingMapsRasterOverlay(
    const std::string& name,
    const std::string& url,
    const std::string& key,
    const std::string& mapStyle /*= ThirdPartyMapsStyle::AERIAL*/,
    const std::string& culture /*= ""*/,
    const CesiumGeospatial::Ellipsoid&
        ellipsoid /*= CesiumGeospatial::Ellipsoid::WGS84*/,
    const RasterOverlayOptions& overlayOptions /*= {}*/)
    : ThirdPartyMapsRasterOverlay(
          name,
          url,
          key,
          mapStyle,
...
Future<std::unique_ptr<RasterOverlayTileProvider>>
FBingMapsRasterOverlay::createTileProvider(
    const AsyncSystem& asyncSystem,
    const std::shared_ptr<IAssetAccessor>& pAssetAccessor,
    const std::shared_ptr<CreditSystem>& pCreditSystem,
    const std::shared_ptr<IPrepareRendererResources>& pPrepareRendererResources,
  ...
  metadataUrl =
      CesiumUtility::Uri::addQuery(metadataUrl, "incl", "ImageryProviders");
  metadataUrl = CesiumUtility::Uri::addQuery(metadataUrl, "key", this->_key);
  metadataUrl = CesiumUtility::Uri::addQuery(metadataUrl, "culture", this->_culture);
  metadataUrl = CesiumUtility::Uri::addQuery(metadataUrl, "uriScheme", "https");
  ...
  return pAssetAccessor->get(asyncSystem, metadataUrl)
      .thenInMainThread(
          [metadataUrl, pLogger, handleResponse, reportError](
            ...
            // If the response successfully created a tile provider, cache
            // it.
            if (pProvider) {
              sessionCache[metadataUrl] = std::vector<std::byte>(
                  pResponse->data().begin(),
